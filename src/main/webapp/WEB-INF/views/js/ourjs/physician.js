...
function loadPhysician() {
	$.ajax({
		type : "GET",
		url : "/EMR_Admin/physicianList",
		success : function(data) {
			loadPhysicianData(data);
		},
		dataType : "json",
	});
}

function loadPhysicianData(physicians) {
	var dataSet = [];
	for (var i=0;i<physicians.length;i++){
		var physician = [];
		physician.push(physicians[i].physicianName);
		physician.push(physicians[i].physicianGender);
		physician.push(convertMillisecondsToDate(physicians[i].physicianBirthday));
